...
- [m序列优选对生成算法](./Data/References/m序列优选对及平衡Gold码序列_辛肖明.pdf)[^2]
  ...
  ​	具体代码实现可见同文件夹下[getoptpairs.m](./getoptpairs.m)函数。
...
​	具体的m序列初态估计代码见同文件夹下的[mStateEst.m](./mStateEst.m)。
...
​	在估计Gold序列的初态时，估计方法与m序列相同，只是移位寄存器的个数设置为2L，并且跟多项式估计一样进行分段。具体代码见同文件夹下的[GoldStateEst.m](./GoldStateEst.m)。
